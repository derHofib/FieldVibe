...
export function UpdatePage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl font-bold text-slate-900 dark:text-stone-100">Update</h1>
      ...
      {isLoading && <p className="text-sm text-slate-500 dark:text-stone-400">Lädt…</p>}
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-md border border-slate-200 bg-white p-4 dark:border-stone-700 dark:bg-stone-900">
            ...
            <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 text-sm">
              <dt className="text-slate-500 dark:text-stone-400">Commit</dt>
              <dd className="font-mono text-slate-900 dark:text-stone-100">{data.deployed_commit}</dd>
              <dt className="text-slate-500 dark:text-stone-400">Branch</dt>
              <dd className="font-mono text-slate-900 dark:text-stone-100">{data.branch}</dd>
            </dl>
          </div>
          ...
          <div className="rounded-md border border-slate-200 bg-white p-4 dark:border-stone-700 dark:bg-stone-900">
            ...
            {data.latest_commit_sha ? (
              <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 text-sm">
                <dt className="text-slate-500 dark:text-stone-400">Commit</dt>
                <dd className="font-mono text-slate-900 dark:text-stone-100">
                ...
                <dt className="text-slate-500 dark:text-stone-400">Nachricht</dt>
                <dd className="text-slate-900 dark:text-stone-100">{data.latest_commit_message}</dd>
                <dt className="text-slate-500 dark:text-stone-400">Datum</dt>
                <dd className="text-slate-900 dark:text-stone-100">
                  {formatDatum(data.latest_commit_date)}
                </dd>
              </dl>
            ) : (
              <p className="text-sm text-slate-500 dark:text-stone-400">Nicht ermittelbar.</p>
            )}
          </div>
          ...
          <div className="rounded-md border border-slate-200 bg-white p-4 dark:border-stone-700 dark:bg-stone-900">
            ...
            <p className="mb-2 text-sm text-slate-600 dark:text-stone-400">
