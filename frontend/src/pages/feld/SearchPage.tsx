  ...
  return (
    <div className="space-y-4">
      ...
      {!q && tags && tags.length > 0 && (
        <div>
          <h2 className="mb-2 text-sm font-semibold text-slate-500">Tags</h2>
          <div className="flex flex-wrap gap-2">
            {tags.map((tag) => (
              <button
                key={tag.id}
                onClick={() => setQ(`#${tag.label}`)}
                className="btn-touch rounded-full bg-white px-3 py-1.5 text-sm text-slate-600 shadow-sm"
              >
                #{tag.label}
              </button>
            ))}
      ...
      {q && (
        <div className="space-y-2">
          {isFetching ? (
            <p className="text-center text-slate-500">Suche…</p>
            ...
            results.treffer.map((hit) => (
              ...
              >
                <div>
                  <div className="text-sm font-medium text-slate-800">{hit.titel}</div>
                  {hit.subtitel && <div className="text-xs text-slate-400">{hit.subtitel}</div>}
                </div>
            ...
            <p className="text-center text-slate-500">Keine Treffer.</p>
          )}
        </div>
      )}
    </div>
