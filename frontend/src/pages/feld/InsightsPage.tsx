...
function Kachel({ label, wert }: { label: string; wert: string }) {
  return (
    <div className="border border-ind-line bg-ind-bg p-4">
      <div className="text-xs font-medium text-ind-ink-3">{label}</div>
      <div className="mt-1 text-2xl font-bold text-ind-ink">{wert}</div>
    </div>
  );
}
...
export function InsightsPage() {
  ...
  if (isLoading || !insights) return <p className="text-center text-ind-ink-3">Lädt…</p>;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border border-ind-line bg-ind-bg p-4">
        <h2 className="mb-2 text-sm font-semibold text-ind-ink-3">Vorgänge nach Status</h2>
        <div className="space-y-1.5">
          {Object.entries(insights.vorgaenge_nach_status).map(([status, count]) => (
            <div key={status} className="flex items-center gap-2">
              <span className="w-32 shrink-0 text-sm text-ind-ink-2">
              ...
              <div className="h-4 flex-1 overflow-hidden rounded-full bg-slate-100 dark:bg-stone-800">
                <div
                  className="h-full rounded-full bg-ind-btn-bg"
                  style={{ width: `${gesamtVorgaenge > 0 ? (count / gesamtVorgaenge) * 100 : 0}%` }}
                />
              </div>
              <span className="w-8 shrink-0 text-right text-sm font-medium text-ind-ink-2">
                {count}
              </span>
            </div>
          ))}
      ...
      <div className="border border-ind-line bg-ind-bg p-4">
        <h2 className="mb-2 text-sm font-semibold text-ind-ink-3">Export</h2>
