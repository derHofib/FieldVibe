...
# Endpunkt statt einzelner Felder eingeschränkt.
router = APIRouter(
...
async def list_vertraege(
    ...
    stmt = select(Vertrag).order_by(Vertrag.bezeichnung)
    if kunde_id:
...
@router.post("", response_model=VertragRead, status_code=status.HTTP_201_CREATED)
...
@router.patch("/{vertrag_id}", response_model=VertragRead)
